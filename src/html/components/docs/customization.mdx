...
Need more than Bootstrap's eight theme colours — a navy sidebar, a teal widget row, your brand's blue as a first-class `.text-bg-brand`? That is what the opt-in [extended palette](colors.html) is for: twelve designed colours plus a one-line Sass hook (`$lte-palette-custom`) for your own, in a separate stylesheet so it costs nothing unless you load it.
