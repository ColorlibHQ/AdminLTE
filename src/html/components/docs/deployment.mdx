...
| Select2 theme *(optional)* | `dist/css/adminlte-select2.css` | **`dist/css/adminlte-select2.min.css`** |

`adminlte-select2.css` is opt-in and only needed if your app loads [Select2](integrations.html#select2-compatibility-theme); it is not part of `adminlte.css`. `adminlte-docs.css` styles the documentation pages only — never ship it.
...
<link
  rel="stylesheet"
  href="https://cdn.jsdelivr.net/npm/admin-lte@4.8.3/dist/css/adminlte.min.css"
